...
Texture2D BumpTexture1 : register(t2);
Texture2D Bumptexture2 : register(t3);
...
float4 PSMain(in VS_OUTPUT input) : SV_Target
{
  ...
  //  float3 bump1 = (1 - texweight) * 4 * (BumpTexture1.Sample(TextureSampler, input.tex) - 0.5f);
  //  float3 bump2 = texweight * 4 * (Bumptexture2.Sample(TextureSampler, input.tex) - 0.5f);

    float3 bump1 = (1 - texweight) * 4 * (BumpTexture1.Sample(TextureSampler, input.tex) - 0.5f);
    float3 bump2 = texweight * (Bumptexture2.Sample(TextureSampler, input.tex) - 0.5f);
    float3 bumpSum = bump1 + bump2;
   // float3 normalVector = input.normal + (bump1.x + bump2.x) * input.tangent + (bump1.y + bump2.y) * input.binormal;
  // float3 normalVector = input.normal + bump1.x * input.tangent + bump1.y * input.binormal;
   // float3 normalVector = input.normal + bumpSum.x * input.tangent + bumpSum.y * input.binormal;
    float3 normal1 = input.normal + bump1.x * input.tangent + bump1.y * input.binormal;
    float3 normal2 = input.normal + bump2.x * input.tangent + bump2.y * input.binormal;
    float3 normalVector = normal1 * normal2;
  ...
  //  float texweight = 1 / (1 + exp(terrainheight - TextureBoundary.x));
    ...
    float4 pixelcolour = sampledTextureColour * lightIntensity4f;
